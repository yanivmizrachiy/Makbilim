  ...
  it('a role is drawn the same way on every page', () => {
    ...
    for (const d of diagrams) {
      for (const mark of renderedMarks(d.svg)) {
        ...
        // 'angle-mark--index' is a presentation mode, not a style: a crossing whose four angles all
        // carry bare numbers draws the numbers without arcs (no bullseye). It may only occur there.
        if (mark.className.includes('angle-mark--index')) {
          expect(mark.role, `${d.taskId}: arc-less numbering mark`).toBe('marked');
          expect(d.svg, `${d.taskId}: arc-less numbering mark has a label`).toMatch(/class="angle-label-text angle-label-text--index"/);
        }
        const classes = classesByRole.get(mark.role) ?? new Set<string>();
        classes.add(mark.className.replace(' angle-mark--index', ''));
