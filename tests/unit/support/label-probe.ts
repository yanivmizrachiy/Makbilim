 ...
 */
import { segmentNearRect, type Point, type Rect, type Segment } from '../../../src/geometry/core';
...
/**
 * A label counts as on a line when a line passes within this distance of its ink box (glyph ink
 * from the label face's metrics, plus its white halo).
 */
const ON_LINE_CLEARANCE = 2;
const PX_PER_MM = 96 / 25.4;
const PX_PER_PT = 96 / 72;

/** The ink of a label (glyphs at their printed size plus the white halo), in diagram units. */
function labelInk(source: string, x: number, baseline: number, fontPt: number): Rect {
  const shaped = shapeLabel(source);
  const fontPx = fontPt * PX_PER_PT;
  const halo = (T.label.haloMm / 2) * PX_PER_MM;
  const half = (shaped.width / 2) * fontPx;
  return {
    left: x - half - halo,
    right: x + half + halo,
...
export type AngleLabelProbe = {
  ...
  at: string;
  point: Point;
  /** The label's ink: its glyphs at printed size plus the white knockout halo. */
  plate: Rect;
...
export function angleLabels(svg: string): AngleLabelProbe[] {
  ...
  // One chunk per mark group: from its opening tag to the next mark group (or the end). Arc-less
  // numbering marks (angle-mark--index) name no swept angle and are measured by the engine tests.
  const starts = [...svg.matchAll(/<g class="(angle-mark[^"]*)"((?: data-[a-z-]+="[^"]*")*)>/g)];
  return starts
    .map((m, i) => ({
      className: m[1]!,
      at: /data-angle-at="([^"]+)"/.exec(m[2]!)?.[1] ?? '',
      body: svg.slice(m.index + m[0].length, starts[i + 1]?.index ?? svg.length),
    }))
    .filter(group => group.body.includes('class="angle-label-text"') && !group.className.includes('angle-mark--index'))
    .map(({ at, body }) => {
      const text = /<text class="angle-label-text" x="([^"]+)" y="([^"]+)"[^>]*data-label="([^"]*)"/.exec(body);
      ...
      if (!text || !arc || !centre) throw new Error(`could not read the angle label at ${at}`);
      ...
      // The label points at its angle when its centre is within the sweep of the drawn arc.
      const start = angle(num(arc[1]), num(arc[2]));
      const sweep = norm(angle(num(arc[3]), num(arc[4])) - start);
      const ink = labelInk(label, point.x, point.y, T.label.anglePt);
      return {
        text: label,
        at,
        point,
        ...
        coversChevron: chevrons.some(stroke => segmentNearRect(stroke, ink, 1)),
      };
    });
}
...
export function chevronTouchesArc(svg: string, clearance = 2): boolean {
  ...
  const arcs = starts.flatMap((m, i) => {
    ...
    const at = /data-angle-at="([^"]+)"/.exec(m[1]!)?.[1] ?? '';
    const centre = crossings[CROSSING_ORDER.indexOf(at as (typeof CROSSING_ORDER)[number])]!;
